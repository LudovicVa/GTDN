...
//		include 'model.php';
//		$this->model = new ConfigAdminModel();
//
//		include 'view.php';
//		$this->setView(new ConfigAdminView());
	}
    ...
    public function launch_2() {
        ...
        switch ($data['command']) {
            ...
            //Default values
            case 'GET_DEFAULT_FRONT':
                if ($default = WConfig::get('route.default')) {
                    $this->view->push_content("GET_DEFAULT_FRONT", $default);
                } else {
                    $this->view->error('installer', $data['installer'], 'Fatal Error', 'Error while retrieving default front app.');
                }
                break;

            case 'GET_DEFAULT_ADMIN':
                if ($default = WConfig::get('route.admin')) {
                    $this->view->push_content("GET_DEFAULT_ADMIN", $default);
                } else {
                    $this->view->error('installer', $data['installer'], 'Fatal Error', 'Error while retrieving default admin app.');
                }
                break;

            case 'GET_TIMEZONE':
                if ($default = WConfig::get('config.timezone')) {
                    $this->view->push_content("GET_TIMEZONE", $default);
                } else {
                    $this->view->error('installer', $data['installer'], 'Fatal Error', 'Error while retrieving default timezone.');
                }
                break;

            case 'GET_BASE':
                if ($default = WConfig::get('config.base')) {
                    $this->view->push_content("GET_BASE", $default);
                } else {
                    $this->view->error('installer', $data['installer'], 'Fatal Error', 'Error while retrieving the site base URL.');
                }
                break;

            case 'GET_SITE_NAME':
                if ($default = WConfig::get('config.site_name')) {
                    $this->view->push_content("GET_SITE_NAME", $default);
                } else {
                    $this->view->error('installer', $data['installer'], 'Fatal Error', 'Error while retrieving the site name.');
                }
                break;

            case 'GET_DEFAULT_THEME':
                if ($default = WConfig::get('config.theme')) {
                    $this->view->push_content("GET_DEFAULT_THEME", $default);
                } else {
                    $this->view->error('installer', $data['installer'], 'Fatal Error', 'Error while retrieving the theme.');
                }
                break;

            case 'GET_LANG':
                if ($default = WConfig::get('config.lang')) {
                    $this->view->push_content("GET_LANG", $default);
                } else {
                    $this->view->error('installer', $data['installer'], 'Fatal Error', 'Error while retrieving the default language.');
                }
                break;

            case 'GET_EMAIL':
                if ($default = WConfig::get('config.email')) {
                    $this->view->push_content("GET_EMAIL", $default);
                } else {
                    $this->view->error('installer', $data['installer'], 'Fatal Error', 'Error while retrieving the email.');
                }
                break;
        }
